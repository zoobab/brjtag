// *********************************************************************************************
//
//  brtag.c - Broadcom EJTAG Debrick Utility v1.9n - by hugebird. 5/11/2010
//
//
//////////////////////////////////////////////////////////////////////////////////////////
...
cable_list_type cable_list[]={
 {0, XILINX  , 0         , 0, 0      ,"Parallel port type(DLC5/WIGGLER)" },  //always the default cable, must on the top
 {1, FT2232D , 0x14575118, 1, ftinit ,"FT2232C/D based USB cable(OpenMoko,JTAGkey,OpenJTAG)" },
 {2, JLINK   , 0x13660101, 1, jlinit ,"SEGGAR J-Link EMU"     },
 {3, USBASP  , 0x16C005DF, 1, uinit  ,"HID-BRJTAG v1.xx(USBASP)" },
 {4, STM32   , 0x04835750, 1, stinit ,"HID-BRJTAG v2.xx(STM32F10x/SAM7S)" },
...
/* ********* Broadcom Wifi Chips ***********************************/    
  ...
  { 0x4716,  C_BRCM, 8, __LE,     0,          0, 0,"BCM4716"       },
...
static int det_instr(void)
{
  ...
  if (!cable_intf){
   ...
   for (i = 1; i <32; i++) 
    if ( (out_data >> i)&1 ) return (i);
   
   return 0;

}

static void set_instr(DWORD instr)
{
  int i;
...
static void chip_detect(void)
{
  ...
  id = ReadData();
    ...
    cmfr = GETMFRID(id);
  printf("Done\n\n");
  printf("Detected IR Length is %d\n\n",instruction_length);
       ...
       while (processor_chip->chip_id)
        {
          ...
          if ((cdid == processor_chip->chip_id)&&(cmfr == processor_chip->chip_mfrid))
            {
              instrlenbk = processor_chip->instr_length;
              if ( instruction_length != instrlenbk )
               	printf("Warning: detected IR Length isn't same as knew!\n\n");
  ...
  if (id == 0 || id == 0xFFFFFFFF){
    ...
    cpumfr_type* cmfr_dscr = cpu_mfr_list;
     ...
     if(!cmfr_dscr->id) cmfr_dscr = cpu_mfr_list;

	   printf("    CPU Manufature:%s(%3.3X)\n", cmfr_dscr->name,cmfr);
...
static void show_usage()
{
  ...
  printf( "\n\n");
  printf( " USAGE: Brjtag /showflashlist\n");
  printf( " USAGE: Brjtag [parameter] </noreset> </noemw> </nocwd> </nobreak></LE|BE>\n"
  ...
  while (processor_chip->chip_id)
    {
      ...
      if(!cmfr_dscr->id) processor_chip->mfrdscr_idx = cpu_mfr_list;
      processor_chip++;
...
int main(int argc, char** argv)
{
  ...
  printf("\n");
  printf("        ===============================================\n");
  printf("         Broadcom EJTAG Debrick Utility v1.9o-hugebird \n");
  printf("        ===============================================\n\n");
  ...
  if (argc > 2)
    {
      ...
      while (j < argc)
       ...
       /* **    Ejtag Access   ** */
        	...
        	else if (strcasecmp(choice,"/wx8")==0)            prg_x8 = 1;
...
//        else if (strncasecmp(choice,"/sz:",4)==0)        {  sz = strtoul(((char *)choice + 4),NULL,10) & 0x3;}
        ...
        /* **    Others   ** */  
          ...
          else if (strcasecmp(choice,"/wiggler")==0)         cable_type = WIGGLER;
